...
#[tauri::command]
pub fn get_loaded_report(loaded_report: tauri::State<Arc<Mutex<Option<Report>>>>) -> Option<Report> {
...
#[tauri::command]
pub fn new_report(conn: tauri::State<Arc<Mutex<duckdb::Connection>>>, loaded_report: tauri::State<Arc<Mutex<Option<Report>>>>, report: NewReport) -> Result<Report, String> {
...
#[tauri::command]
pub fn load_report(conn: tauri::State<Arc<Mutex<duckdb::Connection>>>, loaded_report: tauri::State<Arc<Mutex<Option<Report>>>>, id: String) -> Result<Report, String> {
    let conn_arc_clone = Arc::clone(&conn);
    let conn = conn_arc_clone.lock().unwrap().try_clone().unwrap();
    ...
    let mut stmt = conn.prepare("UPDATE reports SET last_access_tsz = now() WHERE id = ? RETURNING *;").unwrap();
