<script lang="ts">
    ...
    import NetNodeModal from "$lib/components/NetNodeModal.svelte";
    
    
    const sigmaNodeIcons = {
        Unknown: '',
    ...
    async function addNode(event: any) {
        ...
        let node: NetNode = {
            name: formData.get("name")?.toString() || "Node",
            type: formData.get("type") || "Unknown",
            interfaces: {},
            services: []
        };
        await invoke("add_net_node", { node });
    ...
    $effect(() => {
        ...
        for (const [i, n] of Object.entries(netGraph.nodes)) {
            ...
            sigmaGraph.addNode(i, {
                ...
                image: URL.createObjectURL(new Blob([sigmaNodeIcons[n.type]], { type: "image/svg+xml" })),
                color: "gray",
                pictoColor: "black",
            });
        }
...
<div class="z-1 position-absolute top-50 end-0 m-2 vstack gap-2" hidden={selectedNode === null}>
   	<div class="card border-0 shadow-lg p-2 vstack gap-2">
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#info-node" aria-label="Node info"><i class="bi bi-info-lg"></i></button>
...
{#if selectedNode !== null}
	<NetNodeModal index={Number(selectedNode)} node={netGraph.nodes[Number(selectedNode)]} />
{/if}
